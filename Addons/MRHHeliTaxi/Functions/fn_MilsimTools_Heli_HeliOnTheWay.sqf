...
[_LZ] spawn {
	...
	_player sideChat format ["De %3 pour %2: Demandons évacuation aérienne aux coordonées %1",([_LZ] call MRH_fnc_MilsimTools_Core_realisticGrid),groupID group _myvec,(_player getVariable "MRH_MilsimTools_Core_PlayerIntel") select 0];
	sleep 4;
		playSound "MRH_HeliTaxi_HeliEnRoute";
	_myvec sideChat format ["De %2 pour %3: Bien reçu, hélicoptère en route vers coordonées %1",([_LZ] call MRH_fnc_MilsimTools_Core_realisticGrid),groupID group _myvec,(_player getVariable "MRH_MilsimTools_Core_PlayerIntel") select 0];
	...
	while { ( (alive _myvec) && !(unitReady _myvec) ) } do
{
	...
	_action = ["MRH_Taxi_Go","Donner une destination","",_statement,_condition] call ace_interact_menu_fnc_createAction;  
	[player, 1, ["ACE_SelfActions"], _action] call ace_interact_menu_fnc_addActionToObject;
	};
};
