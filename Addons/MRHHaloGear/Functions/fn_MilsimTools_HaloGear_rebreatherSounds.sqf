...
if (!_playRebreather && !(goggles player in _forcedplayItems)) exitWith {diag_log format ["MRH_MilsimTools_HaloGear Rebreather - playsounds deactivated for item class %1",_gogglesClass]};
_index = ["1","2","3","4","5","6","7"];
_isGoggles = false;
if (_gogglesClass == goggles player) then {_isGoggles = true};
if (!_isGoggles &&(_gogglesClass != headgear player)) exitWith {diag_log "MRH_MilsimTools_HaloGear - Object is neither facewear nor goggles nos sounds played"};
