...
case "Warning" : {      
					if (goggles player == "MRH_HaloMask") then {
						...
						while {(((getPosASL player) select 2)> _AADTriggerAltitude)} do {_ctrl ctrlSetFade 0; _ctrl ctrlCommit 0.1; _ctrl ctrlSetFade 1; _ctrl ctrlCommit 0.1};
						ctrlDelete _ctrl
						};
					};
				for "_i" from 0 to 3 do {playSound "MRH_fiveBeeps";uisleep 1;};
				};
case "SelfOp" : {
	if (goggles player == "MRH_HaloMask") then {
						[] spawn {
